...
class md5_t {
	...
	inline void md5_process(const uint8_t *data /*[64]*/)
	{
		uint32_t
...
public:
	...
	inline void append(const void *data, size_t nbytes) {
	...
	inline const md5_res_t* finish() {
		...
		for (i = 0; i < 8; ++i)
			data[i] = (uint8_t)(state.count[i >> 2] >> ((i & 3) << 3));
		/* Pad to 56 bytes mod 64. */
		append(pad, ((55 - (state.count[0] >> 3)) & 63) + 1);
		/* Append the length. */
		append(data, 8);
		for (i = 0; i < 16; ++i)
			result.digit[i] = (uint8_t)(state.abcd[i >> 2] >> ((i & 3) << 3));

		return &result;
	}

	// Вычисление MD5
	inline const md5_res_t* calc(const void *data, size_t nbytes) {
		init();
		append(data, nbytes);
		return finish();
	}

	inline const md5_res_t* calc(const char *str) {
